...
public class AuthController : ControllerBase
{
    ...
    public async Task<IActionResult> Register(RegisterRequest request, CancellationToken ct)
        => StatusCode(201, await _mediator.Send(new RegisterCommand(request), ct));
    ...
    public async Task<IActionResult> Login(LoginRequest request, CancellationToken ct)
        => StatusCode(201, await _mediator.Send(new LoginCommand(request), ct));

    /// <summary>Exchange a refresh token for a new token pair (rotation)</summary>
    /// <response code="201">Returns new access token and refresh token</response>
    ...
    public async Task<IActionResult> Refresh(RefreshRequest request, CancellationToken ct)
        => StatusCode(201, await _mediator.Send(new RefreshCommand(request), ct));
